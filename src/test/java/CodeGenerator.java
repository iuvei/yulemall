...
//        name.forEach(item->System.out.println(item.getName()));
    ...
    public static String gainTableName(String tableName) {
        try {
            ...
            //3.通过数据库的连接操作数据库，实现增删改查（使用Statement类）
            Statement st=conn.createStatement();
            ResultSet rs=st.executeQuery("SELECT table_name ,table_comment FROM information_schema.TABLES WHERE table_schema = 'yulemall'and TABLE_NAME ='"+tableName+"'"+"");
            //4.处理数据库的返回结果(使用ResultSet类)

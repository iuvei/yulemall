layui.define(['table', 'form'], function (exports) {
    ...
    table.render({
        ...
        , cols: [[
            ...
            , {field: 'bankCard', title: ' 银行卡',edit: 'text', sort: true}
            ...
            , {title: '操作', width: 160, align: 'center', fixed: 'right', toolbar: '#table-bank-toolbar'}//设置表格工具条的名称
        ]]
        , page: true//开启分页
        , limit: 20
    ...
    //++++++++++监听工具条操作开始++++++++++
    table.on('tool(LAY-bank-list)', function (obj) {//表格的名称
        var data = obj.data;
        if (obj.event === 'edit') {//匹配工具栏的edit字段
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('bank/form', data).done(function () {//表单的路由
                        form.render(null, 'bank-form');//读取表单的信息
                        //监听提交
                        form.on('submit(bank-form-submit)', function (data) {//form 表单提交的按钮
                            ...
                            $.ajax({
                                ...
                                success: function () {//成功回调
                                    layui.table.reload('LAY-bank-list'); //重载表格
    ...
    var active = {
        add: function () {
            admin.popup({
                ...
                , area: ['550px', '550px']//设置弹出框大小
                , success: function (layero, index) {
                    view(this.id).render('bank/form').done(function () {
                        //监听提交
    ...
    $('.layui-btn.bank-form').on('click', function() {var type = $(this).data('type');
        active[type] ? active[type].call(this) : '';
    });
    //**********新增结束**********

    //==========搜索开始==========
    form.render(null, 'lay-admin-bank-form');
    form.on('submit(LAY-bank-back-search)',
        function(data) {

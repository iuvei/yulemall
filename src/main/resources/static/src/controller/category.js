layui.define(['table', 'form'], function (exports) {
    ...
    table.render({
        ...
        , cols: [[
             {field: 'id',  title: '编号'}
            , {field: 'parentCategory', title: ' 一级分类',edit: 'text', sort: true}
            ...
            , {title: '操作', width: 160, align: 'center', fixed: 'right', toolbar: '#table-category-toolbar'}//设置表格工具条的名称
        ]]
        , page: true//开启分页
        , limit: 20
    ...
    //**********表格显示开始***********
     ...
     //<<<<<<<<<<<<<<<监听单元格编辑开始<<<<<<<<<<<<<<<
    table.on('edit(LAY-category-list)', function(obj){
        var data = obj.data //得到所在行所有键值
            $.ajax({
                type: "POST", //请求方式 post
                dataType: 'json', //数据类型 json
                contentType: "application/json; charset=utf-8",
                url: "/category/update", // 请求地址
    ...
    //++++++++++监听工具条操作开始++++++++++
    table.on('tool(LAY-category-list)', function (obj) {//表格的名称
        var data = obj.data;
        if (obj.event === 'edit') {//匹配工具栏的edit字段
            admin.popup({
                ...
                , success: function (layero, index) {
                    view(this.id).render('category/form', data).done(function () {//表单的路由
                        form.render(null, 'category-form');//读取表单的信息
                        //监听提交
                        form.on('submit(category-form-submit)', function (data) {//form 表单提交的按钮
    ...
    var active = {
        add: function () {
            admin.popup({
                ...
                , area: ['550px', '550px']//设置弹出框大小
                , success: function (layero, index) {
                    view(this.id).render('category/form').done(function () {
                        //监听提交
    ...
    $('.layui-btn.category-form').on('click', function() {var type = $(this).data('type');
        active[type] ? active[type].call(this) : '';
    });
    //**********新增结束**********

    //==========搜索开始==========
    form.render(null, 'lay-admin-category-form');
    form.on('submit(LAY-category-back-search)',
        function(data) {

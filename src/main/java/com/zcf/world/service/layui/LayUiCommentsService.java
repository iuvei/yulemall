package com.zcf.world.service.layui;

import com.zcf.world.pojo.Comments;
import com.zcf.world.mapper.CommentsMapper;
import com.zcf.world.common.layui.LayUiResult;
import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import tk.mybatis.mapper.entity.Example;
import org.springframework.stereotype.Service;
import javax.annotation.Resource;
import java.util.Date;
import java.util.List;
/**
* @author 许宝予
* @date 2019/04/22
*/
@Service
public class LayUiCommentsService{

    @Resource
    private CommentsMapper LayUiCommentsMapper;

    /**
    *新增数据
    */
    public boolean add(Comments comments) {
        if (comments.getCreatTime() == null){
            comments.setCreatTime(new Date());
        }
        if (comments.getUpdateTime() == null){
            comments.setUpdateTime(new Date());
        }
        comments.setDeleted("N");
        return this.LayUiCommentsMapper.insert(comments) == 1;
    }
    /**
    *根据主键删除数据
    */
    public boolean delete(Integer id) {
        Example example = new Example(Comments.class);
        example.createCriteria().andEqualTo("id",id);
        List<Comments> list = this.LayUiCommentsMapper.selectByExample(example);
        if (list.size() != 1){
            return false;
        }
        Comments comments = new Comments();
        comments.setId(list.get(0).getId());
        comments.setDeleted("Y");
        comments.setUpdateTime(new Date());
        return this.update(comments);
    }

    /**
    *根据主键更新非空数据
    */
    public boolean update(Comments comments) {
        comments.setUpdateTime(new Date());
        return this.LayUiCommentsMapper.updateByPrimaryKeySelective(comments) == 1;
    }

    /**
    *分页查询表中数据
    */
    public LayUiResult query(Integer page, Integer limit) {
        PageHelper.startPage(page, limit);
        List<Comments> list = this.LayUiCommentsMapper.selectAll();
        return new LayUiResult("0", "查询成功", new PageInfo<>(list).getTotal(), list);
    }

    /**
    *分页并搜索关键字
    */
     public LayUiResult search(Integer page, Integer limit,String keywords) {
        Example example = new Example(Comments.class);
        example.createCriteria().andLike("id", "%" + keywords + "%");
        PageHelper.startPage(page, limit);
        List<Comments> list = this.LayUiCommentsMapper.selectByExample(example);
        return new LayUiResult("0", "查询成功", new PageInfo<>(list).getTotal(), list);
    }
}
